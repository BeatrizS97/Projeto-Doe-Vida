...
const BloodTypesChart = () => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 lg:p-10 animate-slide-up">
      <div className="flex items-center justify-center mb-8">
        <div className="bg-gradient-to-br from-rose-600 to-red-700 p-3 rounded-full animate-pulse-slow shadow-lg">
          <Droplet className="w-8 h-8 text-white animate-float" />
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-8 pb-6 border-b border-gray-200 animate-fade-in-delay">
        <div className="flex items-center transform hover:scale-110 transition-all">
          ...
          <span className="text-sm text-gray-600 font-semibold">% Doado (Escuro)</span>
        </div>
        <div className="flex items-center transform hover:scale-110 transition-all">
          <div className="w-4 h-4 bg-rose-200 rounded mr-2 shadow-md"></div>
          <span className="text-sm text-gray-600 font-semibold">% Necessário (Claro)</span>
        </div>
      </div>

      {/* Grid de Tipos Sanguíneos com animações */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6 mb-8">
        {bloodTypes.map((blood, idx) => (
          <div
            ...
            className={`relative bg-gradient-to-br ${blood.color} rounded-xl p-4 lg:p-6 text-white cursor-pointer transform transition-all duration-300 animate-slide-up ${
              activeType === blood.type ? 'scale-110 shadow-2xl z-10 rotate-2' : 'hover:scale-105 shadow-lg'
            }`}
          ...
          >
            ...
            <div className="text-center mb-3 relative z-10">
              <div className="text-3xl mb-2 animate-pulse-slow">{blood.icon}</div>
              <h3 className="text-2xl lg:text-3xl font-bold">{blood.type}</h3>
            </div>
            ...
            {/* Tooltip expandido com animação */}
            {activeType === blood.type && (
              <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 -translate-y-full bg-gray-900 text-white px-4 py-2 rounded-lg text-xs whitespace-nowrap z-20 shadow-xl animate-bounce-subtle">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-red-50 to-rose-100 rounded-xl p-4 text-center transform hover:scale-105 hover:rotate-1 transition-all shadow-md animate-slide-up" style={{ animationDelay: '0.8s' }}>
          <div className="flex justify-center mb-2">
            <AlertTriangle className="w-8 h-8 text-red-600 animate-bounce-subtle" />
          </div>
          <p className="text-sm text-gray-600 mb-1 font-semibold">Demanda Crítica</p>
          <p className="text-xl font-bold text-red-600">O-, AB-, B-, AB+</p>
        </div>
        <div className="bg-gradient-to-br from-orange-50 to-yellow-100 rounded-xl p-4 text-center transform hover:scale-105 hover:rotate-1 transition-all shadow-md animate-slide-up" style={{ animationDelay: '0.9s' }}>
          <div className="flex justify-center mb-2">
            <TrendingUp className="w-8 h-8 text-orange-500 animate-pulse" />
          </div>
          <p className="text-sm text-gray-600 mb-1 font-semibold">Mais Doado</p>
          <p className="text-xl font-bold text-orange-600">O+ (38%)</p>
        </div>
        <div className="bg-gradient-to-br from-rose-50 to-pink-100 rounded-xl p-4 text-center transform hover:scale-105 hover:rotate-1 transition-all shadow-md animate-slide-up" style={{ animationDelay: '1s' }}>
          <div className="flex justify-center mb-2">
            <Droplet className="w-8 h-8 text-rose-600 animate-pulse-slow" />
          </div>
          <p className="text-sm text-gray-600 mb-1 font-semibold">Doador Universal</p>
          <p className="text-xl font-bold text-rose-600">O- (4%)</p>
        </div>
      </div>

      {/* Info com animação */}
      <div className="mt-6 bg-gradient-to-r from-rose-50 to-red-50 rounded-lg p-4 transform hover:scale-102 transition-all animate-fade-in-delay-2">
